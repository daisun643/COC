...
bool HelloWorld::init()
{
    ...
    auto welcomeLabel = Label::createWithSystemFont("Welcome to COC Game", "Arial", 48);
    if (welcomeLabel)
    {
    ...
    if (imageItem == nullptr ||
        imageItem->getContentSize().width <= 0 ||
        imageItem->getContentSize().height <= 0)
    {
        // 如果没有图片，创建一个文本菜单项
        closeItem = MenuItemFont::create("Exit", CC_CALLBACK_1(HelloWorld::menuCloseCallback, this));
    }
    else
    {
        closeItem = imageItem;
