...
void DefenseBuilding::attack(Node* target)
{
    ...
    if (distance <= _range)
    {
        // 可以在这里添加攻击动画和伤害计算
        // 例如：创建子弹、播放音效等
        CCLOG("%s attacks target at distance %.2f", _buildingName.c_str(), distance);
    }
}
...
std::string DefenseBuilding::getBuildingInfo() const
{
    ...
    if (_defenseType == DefenseType::WALL)
    {
        snprintf(buffer, sizeof(buffer), 
                 "%s\nLevel: %d/%d\nDefense: %d HP\nUpgrade Cost: %d",
                 _buildingName.c_str(), _level, _maxLevel, _level * 100, getUpgradeCost());
    }
    else
    {
        snprintf(buffer, sizeof(buffer), 
                 "%s\nLevel: %d/%d\nDamage: %d\nRange: %.0f\nSpeed: %.1f\nUpgrade Cost: %d",
                 _buildingName.c_str(), _level, _maxLevel, _damage, _range, _attackSpeed, getUpgradeCost());
    }
