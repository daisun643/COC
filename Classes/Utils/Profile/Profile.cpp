...
void Profile::destroyInstance() { CC_SAFE_DELETE(_instance); }

Profile::Profile()
    : _isLogin(false), _id(0), _name(""), _clansId(0), _clansName("") {}

Profile::~Profile() {}

bool Profile::init() {
  // 先从FileUtils::getInstance()->getWritablePath() +
  // "profile/me.json";尝试加载配置
  ...
  std::string writablePath =
      FileUtils::getInstance()->getWritablePath() + "me.json";
  ...
  std::string resourcesPath =
      FileUtils::getInstance()->fullPathForFilename("profile/me.json");
...
bool Profile::getIsLogin() const { return _isLogin; }

int Profile::getId() const { return _id; }
...
int Profile::getClansId() const { return _clansId; }

const std::string& Profile::getClansName() const { return _clansName; }
...
bool Profile::save() {
  ...
  doc.AddMember("clans-name", rapidjson::Value(_clansName.c_str(), allocator),
                allocator);
