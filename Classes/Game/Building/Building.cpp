...
void Building::upgrade() {
  ...
  CCLOG("Upgraded %s to level %d. HP: %.0f", _buildingName.c_str(), _level,
        _maxHealth);
}
...
void Building::updateHPBar() {
  ...
  // 背景(红)
  _hpBarNode->drawSolidRect(origin, origin + Vec2(width, height),
                            Color4F(1, 0, 0, 1));

  // 前景(绿)
  float ratio = _currentHealth / _maxHealth;
  _hpBarNode->drawSolidRect(origin, origin + Vec2(width * ratio, height),
                            Color4F(0, 1, 0, 1));
}
