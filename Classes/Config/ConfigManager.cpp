...
ConfigManager::ConfigManager() {
    ...
    _townHallConfig.gridPositionX = 22;
    _townHallConfig.gridPositionY = 22;
...
bool ConfigManager::loadJsonFromFile(const std::string& filePath, rapidjson::Document& doc) {
...
bool ConfigManager::loadBackgroundConfig() {
    ...
    // 解析分辨率策略
    if (doc.HasMember("ResolutionPolicies") && doc["ResolutionPolicies"].IsObject()) {
...
bool ConfigManager::loadConstantConfig() {
    ...
    if (doc.HasMember("GridResolutionWidth") && doc["GridResolutionWidth"].IsInt()) {
...
bool ConfigManager::loadBuildingConfig() {
    ...
    if (doc.HasMember("TownHall") && doc["TownHall"].IsObject()) {
        ...
        if (townHall.HasMember("GridPosition") && townHall["GridPosition"].IsArray() && 
            townHall["GridPosition"].Size() >= 2) {
            _townHallConfig.gridPositionX = townHall["GridPosition"][0].GetInt();
            _townHallConfig.gridPositionY = townHall["GridPosition"][1].GetInt();
        }
        
        // 解析AnchorRatio
        if (townHall.HasMember("AnchorRatio") && townHall["AnchorRatio"].IsArray() && 
        ...
        if (townHall.HasMember("defaultLevel") && townHall["defaultLevel"].IsInt()) {
